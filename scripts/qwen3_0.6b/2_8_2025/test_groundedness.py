...
class GroundednessTest(unittest.TestCase):
    ...
    def calculate_groundedness_score(self, question: str, response: list[str], contexts: list) -> tuple[list,float]:
        """calculate Groundedness score"""
        label_statements = self.label_statements(question, response, contexts)
    ...
    def test_groundedness_scores_all_queries(self):
        """Test all queries from CSV and calculate Groundedness scores using RAGAS"""
        
        if os.path.exists(self.File_Output):
            os.remove(self.File_Output)

        total_groundedness_scores = []
        ...
        for index, row in self.df.iterrows():
            with self.subTest(row_index=index):
                ...
                try:
                    ...
                    label_statement,groundedness_score = self.calculate_groundedness_score(query, statements_response, context)
                    groundedness_calc_time = time.time() - groundedness_start_time

                    total_groundedness_scores.append(groundedness_score)
